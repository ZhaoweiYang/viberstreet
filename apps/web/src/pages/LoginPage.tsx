        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
          ...
          {!codeSent ? (
            <form onSubmit={handleSendCode} className="space-y-4">
              ...
              <button type="submit" disabled={loading || !email} className="w-full py-2.5 px-4 bg-purple-600 text-white font-semibold rounded-xl hover:bg-purple-700 transition text-sm disabled:opacity-60">
                {loading ? '...' : t('login.send_code')}
              </button>
            </form>
          ) : (
            <form onSubmit={handleVerify} className="space-y-4">
              <p className="text-sm text-gray-500 text-center">{t('login.code_sent')}</p>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('login.verification_code')}</label>
                <input type="text" value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="000000" maxLength={6} required autoFocus className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-center tracking-[0.3em] font-mono text-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition" />
              </div>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs"><span className="bg-white px-3 text-gray-400">{t('login.or')}</span></div>
          </div>

          <button disabled className="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-600 disabled:opacity-50">
